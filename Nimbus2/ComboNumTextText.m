...
@implementation ComboNumTextText
@synthesize seatLetterLbl,seatRowLbl,observationLbl,failureLbl;
@synthesize failureTxt,seatLetterTxt,seatRowTxt, alertComboView ,commentBtn;
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        // Initialization code
//        seatRowTxt.placeholder=@"Seat-Row";
//        seatLetterTxt.placeholder=@"Letter";
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    failureTxt.selectedTextField.attributedPlaceholder=kDarkPlaceholder([appDel copyTextForKey:@"FAILURE"]);
    seatRowTxt.attributedPlaceholder=kDarkPlaceholder([appDel copyTextForKey:@"SEAT_ROW"]);
    seatLetterTxt.attributedPlaceholder=kDarkPlaceholder([appDel copyTextForKey:@"SEAT_LETTER"]);
...
-(void) seatnumSelectionDone {
    ...
    if(seatIndex1 == 0 && seatIndex2 == 0) {
        ...
        seatRowTxt.text = [NSString stringWithFormat:@"%@%@",[array1 objectAtIndex:seatIndex1],[array2 objectAtIndex:seatIndex2]];
        
        
    }
    seatLetterTxt.text = [NSString stringWithFormat:@"%@",[array3 objectAtIndex:seatIndex3]];
    if([self.delegate respondsToSelector:@selector(valueSelectedInPopover:)])
    {
        [self.delegate valuesSelectedInPopOver:seatRowTxt];
        [self.delegate valuesSelectedInPopOver:seatLetterTxt];
    }
