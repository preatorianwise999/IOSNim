...
-(void)initializeData{
    ...
    if([flightType isEqualToString:NBLAN]){
        languageKey = @"GENERAL_PRE_ECONOMY_SUGEGSTIONS";
    }
    else{
        languageKey = @"GENERAL_BUSINESS_SUGEGSTIONS";
    }
...
//    _headingLabel.frame=CGRectMake(kxposition_NB_LAN_General,kyposition_NB_LAN_General,kwidth_headingLabel_NB_LAN_General,kheight_headingLabel_NB_LAN_General);
    ...
    sectionArray = [[NSMutableArray alloc] initWithObjects:[appDel copyTextForKey:@"GENERAL_VUELO_SUGEGSTIONS"],[appDel copyTextForKey:languageKey],[appDel copyTextForKey:@"GENERAL_ECONOMY_SUGEGSTIONS"], nil];
    self.headingLabel.text = [appDel copyTextForKey:@"Suggestions"];
    [_suggestionsTableView setSeparatorStyle:UITableViewCellSeparatorStyleNone];
...
-(NSString *)getContentInFormDictForView:(id)view
{
    ...
    if(indexPath.section == 0){
        ...
        rowName =[appDel copyEnglishTextForKey:languageKey];
    }
    else{
