//
//  DomGeneralInfoViewController.m
//  LATAM
//
...
#import "DomGeneralInfoViewController.h"
...
@interface DomGeneralInfoViewController ()
{
...
@implementation DomGeneralInfoViewController
@synthesize materialDropdown,baseCrewDropdown,switchValue,ip;
...
- (void)viewDidLoad {
    ...
    [super viewDidLoad];
    self.tableView = _tableView_genralInfo;
    self.ipArray = [[NSMutableArray alloc] init];
    
    self.leastIndexPath = [NSIndexPath indexPathForItem:0 inSection:1];

    _label_heading.textColor=kFontColor;
    [_label_heading setFont:[UIFont fontWithName:kFontName_Robotica_Light size:25.0]];
    self.label_heading.text = [appDel copyTextForKey:@"General Information"];

    CGRect frame =_label_heading.frame;
    frame.origin.y = kyposition_NB_LAN_General;//pass the cordinate which you want
    frame.origin.x = kxposition_NB_LAN_General;
    _label_heading.frame = frame;
    ...
    [_tableView_genralInfo setSeparatorStyle:UITableViewCellSeparatorStyleNone];
...
-(void)switchChanged:(UISwitch *)sw{
    ...
    switchValue = sw.isOn;
    [LTSingleton getSharedSingletonInstance].isDataChanged = YES;
    [LTSingleton getSharedSingletonInstance].enableCells = sw.isOn;
    ...
    // [self initializeIndexPathArray];
    [_tableView_genralInfo reloadData];

    [self cellsForTableView:_tableView_genralInfo];
    ...
    if(view == nil){
        ...
        if(segControl.selectedSegmentIndex == 0){
            for(int section = cell.indexPath.section ;section>=0;section--){
                ...
                rowVal = (section == cell.indexPath.section)?rowVal=cell.indexPath.row - 1:[self.tableView_genralInfo numberOfRowsInSection:section]-1;
                for(int row = rowVal;row >= 0;row--){
                    cell = (OffsetCustomCell *)[self.tableView_genralInfo cellForRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:section]];
            ...
            for(int section = cell.indexPath.section ;section<[self.tableView_genralInfo numberOfSections];section++){
                BOOL isNextFieldFound = NO;
                int rowVal;int tempTag=0;
                rowVal = (section == cell.indexPath.section)?rowVal=cell.indexPath.row+1:0;
                for(int row = rowVal;row < [self.tableView_genralInfo numberOfRowsInSection:section];row++){
                    cell = (OffsetCustomCell *)[self.tableView_genralInfo cellForRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:section]];
                    if([cell viewWithTag:TEXTFIELD_BEGIN_TAG]){
                        tempTag = TEXTFIELD_BEGIN_TAG;
                    }
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    CGPoint pointInTable = [textField.superview.superview convertPoint:textField.frame.origin toView:self.tableView_genralInfo];
    CGPoint contentOffset = self.tableView_genralInfo.contentOffset;
    
    contentOffset.y = (pointInTable.y - kTableViewScrollOffset);
    
    [UIView beginAnimations:@"tableviewAnimation" context:nil];
    [UIView setAnimationDuration:0.5];
    [self.tableView_genralInfo setContentOffset:contentOffset animated:NO];
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    {
        UITableViewCell *cell = (UITableViewCell*)textfieldCellRef;
        NSIndexPath *indexPath = [self.tableView_genralInfo indexPathForCell:cell];
        ip = indexPath;
        [self.tableView_genralInfo scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionMiddle animated:YES];
    }
    [LTSingleton getSharedSingletonInstance].isDataChanged=YES;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 0){
        ...
        ((SwitchCell *)cell).leftLabel.attributedText = [[[appDel copyTextForKey:@"GENERAL_TRAMO_EJECUTADO"] stringByAppendingString:@"*"] mandatoryString];
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!(indexPath.row == 0 && indexPath.section == 0)){
        if (!switchValue || ![LTSingleton getSharedSingletonInstance].enableCells){
            [cell setAlpha:0.6f];
        }else{
            [cell setAlpha:1.0f];
        }
