...
#import "NBEconomyAirportViewController.h"
...
@interface NBEconomyAirportViewController ()
{
...
@implementation NBEconomyAirportViewController
...
- (void)viewDidLoad
{
    ...
    frame.origin.y=5;//pass the cordinate which you want
...
- (void)didReceiveMemoryWarning
{
    ...
    // Dispose of any resources that can be recreated.kc
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *otherNumCameraCellID     = @"OtherNumCameraCellID";
    ...
    OtherNumCamera *otherNumCameraCell = nil;
    ...
    if(indexPath.section == 0)
    {
        ...
        {
            otherNumCameraCell = (OtherNumCamera *)[self createCellForTableView:tableView withCellID:otherNumCameraCellID];
            cell = otherNumCameraCell;
            otherNumCameraCell.indexPath = indexPath;
            
            otherNumCameraCell.reasonTxt.delegate = self;
            otherNumCameraCell.reasonTxt.dataSource = [self getDropDownDataForGroup:@"AIRPORT_BAGGAGE" event:@"AIRPORT_HANDLUGGAGE" content:@"REASON"];
            otherNumCameraCell.reasonTxt.typeOfDropDown = OtherDropDown;
            otherNumCameraCell.reasonTxt.key = [appDel copyEnglishTextForKey:@"REASON"];
            otherNumCameraCell.reasonTxt.selectedTextField.tag = MANDATORYTAG;
            
            //otherNumCameraCell.amountTxt.inputAccessoryView = [self keyboardToolBar];
            otherNumCameraCell.amountTxt.keyboardType = UIKeyboardTypeNumberPad;
            otherNumCameraCell.amountTxt.delegate = self;
            otherNumCameraCell.amountTxt.accessibilityIdentifier = [appDel copyEnglishTextForKey:@"AMOUNT"];
            otherNumCameraCell.amountTxt.tag = MANDATORYTAG;
            otherNumCameraCell.amountTxt.layer.sublayerTransform = CATransform3DMakeTranslation(0, 0, 0);
            
            otherNumCameraCell.reasonLbl.attributedText = [[[appDel copyTextForKey:@"REASON"] stringByAppendingString:@"*"] mandatoryString];
            otherNumCameraCell.amountLbl.attributedText = [[[appDel copyTextForKey:@"AMOUNT"] stringByAppendingString:@"*"] mandatoryString];
            otherNumCameraCell.observationLbl.text = [appDel copyTextForKey:@"OBSERVATION"];
            
            otherNumCameraCell.reasonTxt.selectedTextField.text = [self getContentInFormDictForView:otherNumCameraCell.reasonTxt];

            otherNumCameraCell.amountTxt.text = [self getContentInFormDictForView:otherNumCameraCell.amountTxt];
            otherNumCameraCell.amountTxt.font=[UIFont fontWithName:KFontName size:14.0];

            
            otherNumCameraCell.cameraView.key = @"CAMERA";
            otherNumCameraCell.cameraView.typeOfDropDown = CameraDropDown;
            otherNumCameraCell.cameraView.selectedTextField.hidden = YES;
            otherNumCameraCell.cameraView.delegate = self;
            otherNumCameraCell.cameraView.imageName=[self getContentInFormDictForView:otherNumCameraCell.cameraView];
            if(otherNumCameraCell.cameraView.imageName==nil || [otherNumCameraCell.cameraView.imageName isEqualToString:@""]){
//                [otherNumCameraCell.cameraImageView setImage:[UIImage imageNamed:@"cameraBtn"]];
                [otherNumCameraCell.cameraImageView setImage:[UIImage imageNamed:@"icon_camera_empty"]];

            }
            else{
//                [otherNumCameraCell.cameraImageView setImage:[UIImage imageNamed:@"cameraFill"]];
                [otherNumCameraCell.cameraImageView setImage:[UIImage imageNamed:@"icon_camera_filled"]];
    ...
    else
    {
        ...
        else if(indexPath.row < mallAssignedSeatsArr.count + propertyInfoSeatArr.count + 2)
        {
            ...
            otherCell.reasonTxt.selectedTextField.tag = MANDATORYTAG;
            otherCell.reasonLbl.attributedText = [[[appDel copyTextForKey:@"REPORT"] stringByAppendingString:@"*"] mandatoryString];
        ...
        else
        {
            ...
            //otherTextCell.fullNameTxt.inputAccessoryView = [self keyboardToolBar];
            ...
            otherTextCell.reportLbl.attributedText = [[[appDel copyTextForKey:@"REPORT"] stringByAppendingString:@"*"] mandatoryString];
...
- (OffsetCustomCell *)createCellForTableView:(UITableView *)tableView withCellID:(NSString *)cellID
{
    ...
    else if([cellID isEqualToString:@"OtherNumCameraCellID"]){
        OtherNumCamera *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
        if (cell == nil)
        {
            
            NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"OtherNumCamera" owner:self options:nil];
            cell = [topLevelObjects objectAtIndex:0];
