...
-(void)initializeData {
    ...
    DLog(@"groupArr==%@",groupArr);
    DLog(@"Leg info Boarding ==%@",[LTSingleton getSharedSingletonInstance].flightRoasterDict);
...
frame.origin.y=kyposition_NB_LAN_General;//pass the cordinate which you want
  ...
  //  _headingLabel.frame=CGRectMake(kxposition_NB_LAN_General,kyposition_NB_LAN_General,kwidth_headingLabel_NB_LAN_General,kheight_headingLabel_NB_LAN_General);
    ...
    self.headingLabel.text = [appDel copyTextForKey:@"Boarding"];
    [_boardingTableView setSeparatorStyle:UITableViewCellSeparatorStyleNone];
...
-(NSString *)getContentInFormDictForView:(id)view
{
    ...
    if([[groupArr filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"name == %@",sectionName]] count] > 0) {
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if(section == 0) {
        ...
        if(switchValue) {
            if(greaterThanOneHour)
                return 4 + [motivoArray count] + [actionsArray count];
