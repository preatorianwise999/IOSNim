...
#import "LANWBIFEViewController.h"
...
@interface LANWBIFEViewController ()<UITextFieldDelegate,PopoverDelegate> {
...
@implementation LANWBIFEViewController
...
-(void)initiallizeData
{
    ...
    if(flightRoasterDraft != nil)
    {
        ...
        if([groupArr count] > 0){
            switchValue = [[[[[[[[[[[flightRoasterDraft objectForKey:@"legs"] objectAtIndex:kCurrentLegNumber]  objectForKey:@"reports"] objectAtIndex:0] objectForKey:@"sections"] objectAtIndex:0] objectForKey:@"groups"] objectAtIndex:0] objectForKey:@"singleEvents"] objectForKey:[appDel copyEnglishTextForKey:@"THERE_WERE_MASSIVE_FAILURES_IFE?"]] boolValue];
        }
            ...
            for(NSString *rowName in [[groupDict objectForKey:@"multiEvents"] allKeys])
            {
                for(NSDictionary *rowDict in [[groupDict objectForKey:@"multiEvents"] objectForKey:rowName])
                {
                    ...
                    if([sectionName isEqualToString:[appDel copyEnglishTextForKey:@"MASSIVE_FAILURES"]])
                    {
                        ...
                        if([rowName isEqualToString:[appDel copyEnglishTextForKey:@"INITIAL_STATE_OF_THE_SYSTEM"]] && switchValue)
                        {
...
- (void)viewDidLoad
{
   ...
   __headingLabel.textColor=kFontColor;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *comboTextID1 = @"ComboText1";
    ...
    if(indexPath.row == 0)
    {
        ...
        comboText1 = (ComboText *)[self createCellForTableView:tableView withCellID:comboTextID1];
    ...
    else if(comboText1)
    {
        ...
        cell = comboText1;
        comboText1.indexPath = indexPath;
        comboText1.observationLbl.text = [appDel copyTextForKey:@"OBSERVATION"];
        comboText1.alertComboView.key = [appDel copyEnglishTextForKey:@"OBSERVATION"];
        
        comboText1.alertComboView.typeOfDropDown = AlertDropDown;
        comboText1.alertComboView.selectedTextField.hidden = YES;
        comboText1.alertComboView.delegate = self;
        comboText1.alertComboView.selectedValue = [self getContentInFormDictForView:comboText1.alertComboView];
        comboText1.alertComboView.notesView.text = [self getContentInFormDictForView:comboText1.alertComboView];
        
        comboText1.reportTxt.delegate = self;
        comboText1.reportTxt.dataSource = [self getDropDownDataForGroup:@"MASSIVE_FAILURES" event:@"INITIAL_STATE_OF_THE_SYSTEM" content:@"SELECT_STATE"];
        comboText1.reportTxt.typeOfDropDown = OtherDropDown;
        comboText1.reportTxt.key = [appDel copyEnglishTextForKey:@"SELECT_STATE"];
        comboText1.reportTxt.selectedTextField.text = [self getContentInFormDictForView:comboText1.reportTxt];
        comboText1.reportLbl.attributedText = [[[appDel copyTextForKey:@"SELECT_STATE"] stringByAppendingString:@"*"] mandatoryString];
        comboText1.reportTxt.selectedTextField.tag = MANDATORYTAG;
        
        
        if([self getContentInFormDictForView:comboText1.alertComboView] == NULL || [[self getContentInFormDictForView:comboText1.alertComboView] isEqualToString:@""])
        {
            [comboText1.commentBtn setImage:[UIImage imageNamed:@"icon_comment_empty"]];
        }
        else
            [comboText1.commentBtn setImage:[UIImage imageNamed:@"icon_comment_filled"]];
    }
...
- (OffsetCustomCell *)createCellForTableView:(UITableView *)tableView withCellID:(NSString *)cellID
{
    ...
    }else if([cellID isEqualToString:@"ComboText1"]) {
