                            ...
                            for (NSString *eventName in [eventDict allKeys]) {
                                ...
                                for (int i=0; i<[eventArray count]; i++) {
                                    ...
                                    for (NSString *contentName in [contentDict allKeys]) {
                                        ...
                                        if ((flight.status.intValue == draft || flight.status.intValue == 0) && [LTSingleton getSharedSingletonInstance].isDataChanged) {
                                            flight.status = [NSNumber numberWithInt:draft];
                                        }
                                        
                                        if (![[contentDict objectForKey:contentName] isEqualToString:@"Select"]) {
